...
//            transition.subtype = .fromLeft  // 👈 slide in from the left
//            tabBarController.view.layer.add(transition, forKey: kCATransition)
//
//            // Switch to 4th tab (index 3)
...
extension TaskListViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // Enable "Start Pomodoro" swipe action
        func tableView(_ tableView: UITableView,
                       trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath)
                       -> UISwipeActionsConfiguration? {
            let startPomodoroAction = UIContextualAction(style: .normal, title: "Pomodoro") { [weak self] _, _, completionHandler in
                guard let self = self else { return }
                let selectedTask = self.tasks[indexPath.row]
                // Store the selected task’s ID so PomodoroViewController can access it
                PomodoroManager.shared.activeTaskID = selectedTask.id
            ...
            startPomodoroAction.backgroundColor = .systemOrange
            return UISwipeActionsConfiguration(actions: [startPomodoroAction])
        }
    ...
    func tableView(_ tableView: UITableView,
                   ...
                   -> UISwipeActionsConfiguration? {
                       ...
                       let focusMusicAction = UIContextualAction(style: .normal, title: "Focus") { [weak self] _, _, completionHandler in
                               guard let self = self else { return }

                               // Go to the 4th tab (Focus Music)
                               UIView.transition(with: self.tabBarController!.view,
                                                 duration: 0.25,
                           ...
                           focusMusicAction.backgroundColor = UIColor.systemBlue
                           focusMusicAction.image = UIImage(systemName: "headphones")

                           // Return config → enables half & full swipe
                           let configuration = UISwipeActionsConfiguration(actions: [focusMusicAction])
